$(document).ready(function() {
  ...
  //queue data
  queue()
    .defer(d3.csv, "../data/ekodata/lan_data.csv", type)
    .defer(d3.csv, "../data/ekodata/kommun_data2018.csv", type)
    .defer(d3.json, "../data/ekodata/kommuner_shape.geojson")
    .defer(d3.json, "../data/ekodata/lan_shape.geojson")
    .defer(d3.csv, "../data/ekodata/vkv.csv", vkvType)
    .defer(d3.json, "../data/ekodata/swe_shape.geojson")
    .defer(d3.csv, "../data/ekodata/landsting.csv", ltType)
    .await(pushData);
  ...
  function getColour(objectResult) {
    return objectResult === null
      ? nodata
      : objectResult >= 0 && objectResult <= 9
      ? red
      : objectResult > 9 && objectResult <= 19
      ? orange
      : objectResult > 19 && objectResult <= 29
      ? yellow
      : objectResult > 29 && objectResult <= 39
      ? lightGreen
      : objectResult > 39
      ? darkGreen
      : nodata;
  ...
  function drawSweLan(lanShape, data) {
    ...
    var lanPath = g
      ...
      .style({
        fill: function(d, i) {
          timeLegendData.push(getColour(data[i]["2018"]));
          defaultLegendData.push(getColour(data[i]["2018"]));
          return getColour(data[i]["2018"]);
        },
        stroke: "#fff",
        "stroke-width": "0.8px",
        opacity: 1
        ...
        if (
          ...
          for (var j = 0; j < lanDataSorted.length; j++) {
            if (lanDataSorted[j].LNKOD == lanId) {
              $lanResult.html("<p>" + lanDataSorted[j]["2018"] + "%</p>");
  ...
  function drawKommuner(lanKomData, lanKomShape, lanId) {
    ...
    var kommunPath = g
      ...
      .style({
        fill: function(d, i) {
          timeLegendData.push(getColour(lanKomData[i]["2018"]));
          return getColour(lanKomData[i]["2018"]);
        },
        "stroke-width": "0.2px",
        stroke: "#fff",
        opacity: 1
          ...
          if (currentView == "trend") {
            ...
            for (var j = 0; j < lanKomData.length; j++) {
              if (lanKomData[j].KOM_IDID == komId) {
                ...
                if (!isNaN(lanKomData[j]["2018"])) {
                  $kommunResult.html("<p>" + lanKomData[j]["2018"] + "%</p>");
                } else {
            ...
            for (var j = 0; j < activeKom.length; j++) {
              if (activeKom[j].KOM_IDID == komId) {
                var activeCol = getColour(activeKom[j]["2018"]); //get colour from result
                ...
                //check for null data and update accordingly
                if (!isNaN(activeKom[j]["2018"])) {
                  $("#komStatResult").html(
                    "<p>" + activeKom[j]["2018"] + "%</p>"
                  );
  ...
  function clicked(d) {
    ...
    if (!isPlaying) {
      currentFrame = "2018"; //reset currentFrame
      $("#graphYear").text("2018"); //update year in trendgraph
  ...
  function defaultLineChart() {
    ...
    $sweResult.html("<p>38%</p>");

    //set up lingraph scales
    lineChartScaleX = d3.scale
      .linear()
      .domain([2008, 2018])
      .range([0, lineChartWidth - 70]);

    lineChartScaleY = d3.scale
      .linear()
      .domain([0, 100])
    ...
    line = d3.svg
      ...
      .x(function(d) {
        return lineChartScaleX(d.year);
      })
      .y(function(d) {
        return lineChartScaleY(d.percent);
      })
      .defined(function(d) {
        return !isNaN(d.percent);
      });
  ...
  function swedenResults() {
    lineChartDefaultData = [
      ...
      { year: 2016, percent: 33 },
      { year: 2017, percent: 36 },
      { year: 2018, percent: 38 }
    ];
  ...
  function drawLanResults(lanData) {
    for (var i = 0; i < lanData.length; i++) {
      ...
      var lanResults = [
        ...
        { year: 2016, percent: lanData[i]["2016"] },
        { year: 2017, percent: lanData[i]["2017"] },
        { year: 2018, percent: lanData[i]["2018"] }
      ];
  ...
  function drawKomResults(komData) {
    ...
    for (var i = 0; i < komData.length; i++) {
      ...
      var komResults = [
        ...
        { year: 2016, percent: komData[i]["2016"] },
        { year: 2017, percent: komData[i]["2017"] },
        { year: 2018, percent: komData[i]["2018"] }
      ];
  ...
  function createSlider() {
    sliderScale = d3.scale.linear().domain([2008, 2018]);
  ...
  function animate() {
    interval = setInterval(function() {
      ...
      if (currentFrame == 2018) {
        isPlaying = false;
        $("#playBtn").attr("src", "media/play_button.png");

        d3.select("#playBtn").classed("pause", false);
  ...
  function sortActiveKommuner(activeKomParam) {
    ...
    for (var i = 0; i < sortedKomResults.length; i++) {
      ...
      if (i != 0) {
        if (sortedKomResults[i]["2018"] == prevResult["2018"]) {
  ...
  function topListResults(sortedKomResults) {
    ...
    var topList = barChart
      .selectAll("g.barTopList")
      .data(sortedKomResults)
      .enter()
      .append("g")
      .attr({
    ...
    textResult
      ...
      .attr({
        x: function(d) {
          if (!isNaN(d["2018"])) {
            return barChartScaleX(d["2018"] + 1);
          } else {
            return 10;
          }
        }
  ...
  function radarChart(data) {
    ...
    var blobWrapper = g.append("g").attr("class", "radarWrapper");
  ...
  function iconClick(clickedIcon) {
    ...
    if (iconId == "trendIcon") {
      ...
      $("#svgArea").append('<p id="graphYear">2018</p>');
      $("#svgArea").append(
  ...
  //trend icon events
  $("#trendIcon").click(function() {
    if (!$(this).hasClass("active")) {
      iconClick(this.getAttribute("id"));
    }
  });

  //vkv icon events
  $("#vkvIcon").click(function() {
    if (!$(this).hasClass("active")) {
      iconClick(this.getAttribute("id"));
    }
  });
  ...
  $("#backToSwe").click(function() {
    ...
    currentFrame = "2018"; //reset currentFrame for time animation
    $("#graphYear").text("2018"); //update year in trendgraph
